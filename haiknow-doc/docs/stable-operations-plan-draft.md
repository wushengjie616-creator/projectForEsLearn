...
首版验收：24 篇原文字符保持一致且所有识别出的西语词可点击；带重音词与连字符词切分测试通过；键盘和移动端可用；未登录、无 Key、超限、超时和提供方异常均安全失败；人工重点词汇在无网络时仍可显示。
...
| 点词释义 | 24 篇识别出的西语词全部可点击且原文未变；登录、Key、限流、超时与错误边界可验证；真实调用返回词元、词性、中英义与语法说明；键盘和触屏可用 |
...
| 优先级 | 事项 | 推荐默认值 | 你需要做什么 |
| --- | --- | --- | --- |
| 已决定 | 账号开放方式 | 每人一个可重复使用的个人邀请码 | 已实现；明文存在 Git 忽略文件，仓库只保存 SHA-256 与用户 UUID |
| 已决定 | Git 远端与未来公开 | GitHub 私有仓库起步，按 public-ready 维护 | 已由项目所有者确认；仍需创建/指定空仓库，并另行授权连接和首次推送 |
| 已决定 | 开源许可证组合 | 代码 MIT；原创文档 CC BY 4.0；第三方与衍生内容逐篇继承许可 | 已由项目所有者确认；仍需确认版权持有人署名后创建正式许可文件 |
...
| 已决定 | 点词覆盖 | 人工重点词汇作稳定支架；任意单词由 DeepSeek 提供带风险标识的上下文释义 | 已由项目所有者确认；上线前需用真实账号抽查准确率、延迟和费用 |
