...
| confirmed | GitHub remote 已存在且仓库已经公开；官方 `main` 通过 Pull Request、CI、`CODEOWNERS` 与 GitHub 规则集治理 | 外部用户可以查看、Fork 和提议修改，只有所有者与明确受邀的维护者可以决定合并；部署凭据仍留在 Git 之外 |
...
| 已决定 | Git 远端与公开治理 | GitHub Public 仓库；外部可提议，指定维护者决定合并 | remote 与公开可见性已经建立；维护者名单、`CODEOWNERS`、`main` 规则集和 CI 共同约束正式代码入口 |
...
批准本草案只授权把它转换为正式计划并拆任务，不自动授权购买服务、创建外部账号、配置 DNS 或生产部署。账号方式已经冻结为项目内个人邀请码，不使用邮箱；GitHub 仓库已经公开，并采用“外部可查看和提议、指定维护者决定合并”的治理方式；许可证组合与 DeepSeek 任意单词释义方向也已确认。
